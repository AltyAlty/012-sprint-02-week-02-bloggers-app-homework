...
describe('Users API ID, body and auth validation checks', () => {
  ...
  it('❌ 001 should not return a list of users without proper basic authorization; GET /api/users', async () => {
    ...
    await request(app).get(correctQuery).expect(HttpStatus.Unauthorized_401);
  ...
  it('❌ 002 should not return a list of users when incorrect pagination settings passed; GET /api/users', async () => {
    ...
    await request(app).get(incorrectQuery1).set('Authorization', adminToken).expect(HttpStatus.BadRequest_400);
    await request(app).get(incorrectQuery2).set('Authorization', adminToken).expect(HttpStatus.BadRequest_400);
    await request(app).get(incorrectQuery3).set('Authorization', adminToken).expect(HttpStatus.BadRequest_400);
    await request(app).get(incorrectQuery4).set('Authorization', adminToken).expect(HttpStatus.BadRequest_400);
  ...
  it('❌ 003 should not create a user without proper basic authorization; POST /api/users', async () => {
    ...
    await request(app).post(SETTINGS.USERS_PATH).send(correctCreateUserData).expect(HttpStatus.Unauthorized_401);
  ...
  it('❌ 005 should not create a user when non-unique login/email passed; POST /api/users', async () => {
    ...
    await request(app)
      ...
      .expect(HttpStatus.InternalServerError_500);
  ...
  it('❌ 006 should not delete a user specified by ID without proper basic authorization; DELETE /api/users/:id', async () => {
    ...
    await request(app).delete(`${SETTINGS.USERS_PATH}/${createdUserId}`).expect(HttpStatus.Unauthorized_401);
